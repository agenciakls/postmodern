...
//			}
			...
			if (!isset($erro)) {
				...
				if ($salvar) { 
					$salvarArquivo = mysqli_query($conectar, sprintf("INSERT INTO arquivos_concluidos (id_pedido, titulo, url, status) VALUES ('%s', '%s', '%s', 'ativo')", $idPedido, $nome, $novoCaminhoSite));
					if ($salvarArquivo) {
						echo 'salvo';
					}
